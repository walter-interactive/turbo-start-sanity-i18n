...
/**
 * Type guard to check if a pathname exists in the mapping
 *
 * @param mapping - The locale mapping to check
 * @param pathname - Pathname to validate
 * @returns True if pathname exists in mapping
 *
 * @example
 * ```typescript
 * if (hasTranslations(mapping, pathname)) {
 *   const translations = mapping[pathname];
 *   // TypeScript knows translations is defined
 * }
 * ```
 */
export function hasTranslations(
  ...
  return pathname in mapping;
}

/**
 * Get translations with fallback
 *
 * Safely retrieves translations for a pathname with an optional fallback.
 * Useful for avoiding undefined checks in components.
 *
 * @param mapping - The locale mapping
 * @param pathname - Pathname to lookup
 * @param fallback - Optional fallback value if pathname not found
 * @returns Translations object or fallback
 *
 * @example
 * ```typescript
 * const translations = getTranslationsOrFallback(
 *   mapping,
 *   pathname,
 *   {} // Empty fallback
 * );
 * ```
 */
export function getTranslationsOrFallback(
  mapping: LocaleMapping,
  pathname: string,
  fallback: LocaleTranslations = {}
): LocaleTranslations {
  return mapping[pathname] ?? fallback;
}
